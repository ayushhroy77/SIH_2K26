"""HTTP client for dispatching findings to the CVGuard Governance Spine."""
...
logger = logging.getLogger("cvguard.dataplane.governance_client")

DEFAULT_GOVERNANCE_URL = os.getenv("INTERNAL_GOVERNANCE_URL", "https://governance:8005")


class GovernanceDispatchError(RuntimeError):
    """Raised when a finding cannot be sealed into the Governance Spine after retries."""
...
async def dispatch_finding_to_governance(
    ...
    Planes stay decoupled (no shared DB writes). Fails loudly if unsealed — never drops a finding.
    ...
    mtls_kwargs = get_httpx_mtls_kwargs(service_name="data-plane")
        ...
        for attempt in range(2):
            try:
                ...
                if response.status_code == 201:
                    ...
                    logger.info(
                        "Finding %s sealed in Governance Ledger ID %s",
                        finding.finding_id,
            ...
            if attempt == 0:
                await asyncio.sleep(0.5)  # Brief backoff before single permitted retry
